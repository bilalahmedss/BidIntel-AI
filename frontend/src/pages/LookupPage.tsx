...
export default function LookupPage() {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    setSearch(true); setResult(null)
    try { setResult(await searchLookup(query.trim())) }
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <form onSubmit={handleSearch} className="flex gap-3 mb-6">
        <div className="flex-1 relative">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search certifications, past projects, capabilities…"
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6">
          ...
          {result.chunks?.length > 0 && (
            <details className="mt-4">
              <summary className="text-xs text-slate-500 cursor-pointer hover:text-slate-300">View {result.chunks.length} source excerpt{result.chunks.length !== 1 ? 's' : ''}</summary>
              <div className="mt-3 space-y-3">
                {result.chunks.map((c: string, i: number) => (
                  <div key={i} className="bg-slate-800 rounded p-3 text-xs text-slate-400 font-mono whitespace-pre-wrap">{c.slice(0, 500)}{c.length > 500 ? '…' : ''}</div>
                ))}
              </div>
            </details>
          )}
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-1">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="font-semibold text-sm mb-4 flex items-center gap-2"><Upload size={14} /> Upload Documents</h2>
            <label className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-6 cursor-pointer transition-colors
              ...
              <span className="text-xs text-slate-400 text-center">{uploading ? 'Uploading & indexing…' : 'Click to upload PDF or TXT'}</span>
        ...
        <div className="col-span-2">
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-800">
              <h2 className="font-semibold text-sm flex items-center gap-2"><FileText size={14} /> Knowledge Base ({docs.length} docs)</h2>
            </div>
            {docs.length === 0 ? (
              ...
              <div className="divide-y divide-slate-800">
                {docs.map((d: any) => (
                  <div key={d.id} className="flex items-center justify-between px-5 py-3 hover:bg-slate-800/50 transition-colors">
                    ...
                    <button onClick={() => { if (confirm('Remove this document?')) deleteMut.mutate(d.filename) }}
